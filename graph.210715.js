...
function createColumnGraph(param) {
	...
	clmn.width			= (step.width * clmn.width);
		...
		x, y, b, h, sTop = (chart.top + zero.half);
			...
			items.map(function(item, index) {
				if (columnOk) {
					...
					x = (step.x + (step.width * 0.5));
					// Top érték zéró-vonal vagy az oszlop teteje:
					y = ((step.top === Infinity) ? (chart.innerHeight + zero.half) : (native ? zero.top : (((largest.item - absItem) * step.top) + zero.half)));
					// Bottom érték az oszlop alja vagy a zéró-vonal:
					b = ((step.top === Infinity) ? 0 : ((native ? (zero.bottom - (absItem * step.top)) : zero.bottom)));
					// Magasság:
