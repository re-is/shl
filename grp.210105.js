function createGraph(graph) {
	...
	let chart = document.getElementsByClassName(graph['chart'])[0],
		items = graph['items'],
		balance = [],
		profit = 0,
		highestChartValue = 0,
		...
		for (sheet in sheets) {
			...
			for (rule in rules) {
				...
				// Main Line ------------------------------->
				if (rules[rule].selectorText === ('.graph-' + graph['chart'] + '-main')) {
					mainLineWidth = (parseInt(rules[rule].style['stroke-width']) || 2);
					m_05 = (mainLineWidth * 0.5);
					opacityM = '1';
				}
				// Zero1 Line ------------------------------>
				if (rules[rule].selectorText === ('.graph-' + graph['chart'] + '-zero1')) {
					z1_05 = ((parseInt(rules[rule].style['stroke-width']) || 2) * 0.5);
					opacity1Z = '1';
				}
				// Zero2 Line ------------------------------>
				if (rules[rule].selectorText === ('.graph-' + graph['chart'] + '-zero2')) {
					z2_05 = ((parseInt(rules[rule].style['stroke-width']) || 2) * 0.5);
					opacity2Z = '1';
				}
				// Trend Line ------------------------------>
				if (rules[rule].selectorText === ('.graph-' + graph['chart'] + '-trend')) {
					trendLineWidth = (parseInt(rules[rule].style['stroke-width']) || 2);
					opacityT = '1';
				}
				// Column ---------------------------------->
				if (rules[rule].selectorText === ('.graph-' + graph['chart'])) {
	...
	let chartWidth = chart.clientWidth,
		...
		hAbs = [], hRel = [], lAbs = [], lRel = [];
				...
				if (opacityC === '1') for (let i = 0; i < items.length; i++) {
					...
					html += '<div class="graph-' + graph['chart'] + '" style="position:absolute;width:' + (stepXC * columnWidth) + 'px;height:' + Math.abs(columnHeight) + 'px;margin-left:' + mleft + 'px;margin-top:' + (chartHeight + mtop) + 'px;transform:translate(-50%, -100%);box-sizing:border-box;"></div>';
					mleft += stepXC;
				}
	...
	// SVG:
	html += ('<svg style="position:absolute;margin-top:' + mainLineWidth + 'px;overflow:visible;z-index:1;opacity:' + opacityM + ';" width="' + chartWidth + '" height="' + chartInnerHeight + '"><polyline points="' + mainPoints + '" class="graph-' + graph['chart'] + '-main"></svg>');
	html += ('<svg style="position:absolute;margin-top:' + mainLineWidth + 'px;overflow:visible;z-index:1;opacity:' + opacity1Z + ';" width="' + chartWidth + '" height="' + chartInnerHeight + '"><polyline points="' + zero1Points + '" class="graph-' + graph['chart'] + '-zero1"></svg>');
	html += ('<svg style="position:absolute;margin-top:' + mainLineWidth + 'px;overflow:visible;z-index:1;opacity:' + opacityT + ';" width="' + chartWidth + '" height="' + chartInnerHeight + '"><polyline points="' + trendPoints + '" class="graph-' + graph['chart'] + '-trend"></svg>');
	html += ('<svg style="position:absolute;overflow:visible;z-index:1;opacity:' + opacity2Z + ';" width="' + chartWidth + '" height="' + chartInnerHeight + '"><polyline points="' + zero2Points + '" class="graph-' + graph['chart'] + '-zero2"></svg>');
	// Integrálás:
