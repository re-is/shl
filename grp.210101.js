function createGraph(graph) {
	...
	let svgOffset = chart.getElementsByTagName('svg')[0].getBoundingClientRect(),
		...
		zeroRel = [[0, Math.round(zeroY)], [chartWidth, Math.round(zeroY)]],
		zeroAbs = [[left, (top + zeroRel[0][1])], [(left + chartWidth), (top + zeroRel[1][1])]],
		trndRel = [[0, Math.round(Number(mainLinePoints[1].split(',')[1]))], [chartWidth, Math.round(Number(mainLinePoints[mainLinePoints.length-2].split(',')[1]))]],
		...
		mainAbs = [], mainRel = [], xy, dayTick = 86400000, today = (new Date().setHours(0,0,0,0) + dayTick); // --> ma éjfél
				...
				for (let i = 0; i < (mainLinePoints.length - 4); i++) {
					...
					// Dátum futtatása:
					if (graph['onDate']) graph['onDate'](new Date(Math.round(today - ((items.length - i) * dayTick))), [x, y], [chart.clientWidth, chart.clientHeight], [left, top]);
				}
