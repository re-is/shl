function createLineGraph(param) {
	...
	for (sheet in sheets) {
		...
		for (rule in rules) {
			if (!rules.hasOwnProperty(rule) || !rules[rule].selectorText) continue;
			// Main Line ------------------------------->
			if (rules[rule].selectorText.search(chart.class + '-main') > -1) {
				main.width = (parseInt(rules[rule].style['stroke-width']) || 2);
				main.half = (main.width * 0.5);
				main.css = true;
			}
			// Trend Line ------------------------------>
			if (rules[rule].selectorText.search(chart.class + '-trend') > -1) {
				trnd.width = (parseInt(rules[rule].style['stroke-width']) || 2);
				trnd.half = (trnd.width * 0.5);
				trnd.css = true;
			}
			// Zero Line ------------------------------->
			if (rules[rule].selectorText.search(chart.class + '-zero') > -1) {
...
function createColumnGraph(param) {
	...
	for (sheet in sheets) {
		...
		for (rule in rules) {
			...
			// Average Line ---------------------------->
			if (rules[rule].selectorText.search(chart.class + '-avg') > -1) {
	...
	clmn.width			= (clmn.width * step.left);
